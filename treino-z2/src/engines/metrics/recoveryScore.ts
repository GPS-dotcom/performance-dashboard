import { metricResult, type MetricResult } from "./metricResult";

export interface RecoveryInputs {
  /** TSB from calculateTrainingLoadSeries -- the primary signal, always required. */
  tsb: number;
...
export function calculateRecoveryScore(inputs: RecoveryInputs): MetricResult<number> {
  const requiredInputs = ["tsb (from calculateTrainingLoadSeries)"];
