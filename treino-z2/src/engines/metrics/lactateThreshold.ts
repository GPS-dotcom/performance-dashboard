...
// Fixed-threshold methods: the simplest, most widely cited way to estimate
// the two lactate turn points from an incremental test, without needing
// curve-fitting (e.g. Dmax) that would be much easier to get subtly wrong.
const LT1_THRESHOLD_MMOL = 2.0; // aerobic threshold
const LT2_THRESHOLD_MMOL = 4.0; // anaerobic threshold / OBLA (Onset of Blood Lactate Accumulation)
...
/**
 * Linear interpolation between the two stages bracketing `targetMmol`.
 * Assumes blood lactate rises monotonically with stage, as it should in a
 * correctly administered incremental test.
 */
function interpolateAtLactate(stages: LactateStage[], targetMmol: number): LactateThreshold | null {
...
function thresholdResult(stages: LactateStage[], targetMmol: number): MetricResult<LactateThreshold> {
...
/** LT1 (aerobic threshold): fixed 2.0 mmol/L blood lactate method. */
export function calculateLT1(stages: LactateStage[]): MetricResult<LactateThreshold> {
  return thresholdResult(stages, LT1_THRESHOLD_MMOL);
}

/** LT2 (anaerobic threshold / OBLA): fixed 4.0 mmol/L blood lactate method. */
export function calculateLT2(stages: LactateStage[]): MetricResult<LactateThreshold> {
  return thresholdResult(stages, LT2_THRESHOLD_MMOL);
}
