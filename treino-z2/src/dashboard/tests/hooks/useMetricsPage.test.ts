...
describe("useMetricsPage", () => {
  ...
  it("assembles a metrics view model once athlete data is ready", () => {
    ...
    if (result.current.state.status === "ready") {
      expect(result.current.state.data.dates).toEqual([]);
    }
  });

  it("exposes the underlying retry function", () => {
    mockState.mockReturnValue({ status: "loading" });
    const { result } = renderHook(() => useMetricsPage());
